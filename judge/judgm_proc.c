...
int judgm_proc_init(){
    ...
    proc_watcher_task = kthread_run(proc_watcher,NULL,"judgm_proc");
...
int judgm_proc_add(unsigned long arg){
    ...
    info->jiffiesstart = jiffies;
    info->jiffiesend = info->jiffiesstart + usecs_to_jiffies(com_proc_add->hardtimelimit);
...
static int proc_watcher(void *data){
	...
	list_for_each_entry_rcu(info,&proc_task_list,list){
	    ...
	    }else if(time_after(jiffies,info->jiffiesend)){
		info->runtime = jiffies_to_usecs((unsigned long)(-((long)info->jiffiesstart - (long)jiffies)));
		info->status = JUDGE_TLE;
		send_sig(SIGKILL,info->task,0);
	    }
	}
